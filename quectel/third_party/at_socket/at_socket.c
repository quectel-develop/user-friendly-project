...
struct at_socket *at_get_socket(int socket)
{
    ...
    struct at_socket *at_sock = QOSA_NULL;

    level = QOSA_HW_INTERRUPT_DISABLE();
...
static size_t at_recvpkt_put(rt_slist_t *rlist, const char *ptr, size_t length, s64_t pkt_type)
{
    at_recv_pkt_t pkt = QOSA_NULL;

    pkt = (at_recv_pkt_t) calloc(1, sizeof(struct at_recv_pkt));
    if (pkt == QOSA_NULL)
    {
...
static size_t at_recvpkt_get(rt_slist_t *rlist, char *mem, size_t len, s64_t *pkt_type)
{
    rt_slist_t *node = QOSA_NULL;
    at_recv_pkt_t pkt = QOSA_NULL;
...
static void at_do_event_changes(struct at_socket *sock, at_event_t event, qosa_bool_t is_plus)
{
    switch (event)
    {
    case AT_EVENT_SEND:
...
static struct at_socket *alloc_socket_by_device(struct at_device *device, enum at_socket_type type, int new_idx)
{
    ...
    if (at_slock == QOSA_NULL)
    {
        /* create AT socket lock */
        status = qosa_mutex_create(&at_slock);
        if (status != QOSA_OK)
        {
    ...
    qosa_mutex_lock(at_slock, QOSA_WAIT_FOREVER);

    /* find an empty at socket entry */
    if (device->socket_ops->at_socket != QOSA_NULL)
    {
    ...
    sock->rcvevent = QOSA_NULL;
    sock->sendevent = QOSA_NULL;
    sock->errevent = QOSA_NULL;
    sock->is_client = QOSA_TRUE;
    sock->sin_port = 0;
    ...
    status = qosa_sem_create(&sock->recv_notice, 0);
    if (status != QOSA_OK)
    {
    ...
    status = qosa_mutex_create(&sock->recv_lock);
    if(status != QOSA_OK)
    {
        LOG_E("No memory for socket receive mutex create.");
        qosa_sem_delete(sock->recv_notice);
        goto __err;
    }

    qosa_mutex_unlock(at_slock);
    return sock;
__err:
    qosa_mutex_unlock(at_slock);
    return QOSA_NULL;
...
static void at_closed_notice_cb(struct at_socket *sock, at_socket_evt_t event, const char *buff, size_t bfsz, s64_t pkt_type);
...
int at_socket(int domain, int type, int protocol)
{
    ...
    QOSA_ASSERT(domain == AF_AT || domain == AF_INET);
...
static int free_socket(struct at_socket *sock)
{
    ...
    if (sock->recv_lock)
    {
        qosa_mutex_delete(sock->recv_lock);
    }
...
static int ipaddr_to_ipstr(const struct sockaddr *sockaddr, char *ipstr)
{
    ...
    snprintf(ipstr, 16, "%u.%u.%u.%u", NIPQUAD(sin->sin_addr.s_addr));
...
{
    QOSA_ASSERT(buff);
    QOSA_ASSERT(event == AT_SOCKET_EVT_RECV);
    ...
    qosa_mutex_unlock(sock->recv_lock);
...
static void at_closed_notice_cb(struct at_socket *sock, at_socket_evt_t event, const char *buff, size_t bfsz, s64_t pkt_type)
{
    QOSA_ASSERT(event == AT_SOCKET_EVT_CLOSED);
...
__exit:
    ...
    if (sock)
    {
        at_do_event_changes(sock, AT_EVENT_SEND, QOSA_TRUE);
    }
...
int at_recvfrom(int socket, void *mem, size_t len, int flags, struct sockaddr *from, socklen_t *fromlen)
{
    ...
    while (1)
    {
        ...
        if (qosa_sem_wait(sock->recv_notice, timeout) < 0)
        {
...
__exit:
    ...
    if (sock != QOSA_NULL)
    {
        ...
        {
            result = recv_len;
            at_do_event_changes(sock, AT_EVENT_RECV, QOSA_FALSE);
            //errno = 0;
            if (!rt_slist_isempty(&sock->recvpkt_list))
            {
                at_do_event_changes(sock, AT_EVENT_RECV, QOSA_TRUE);
...
{
    return at_recvfrom(s, mem, len, flags, QOSA_NULL, QOSA_NULL);
}
...
{
    return at_sendto(socket, data, size, flags, QOSA_NULL, 0);
}
...
int at_getsockopt(int socket, int level, int optname, void *optval, socklen_t *optlen)
{
    ...
    if (optval == QOSA_NULL || optlen == QOSA_NULL)
    {
...
static u32_t ipstr_atol(const char* nptr)
{
    u32_t total = 0;
    char sign = '+';
    /* jump space */
...
static u32_t ipstr_to_u32(char *ipstr)
{
    char ipBytes[4] = { 0 };
    u32_t i;
    ...
    for (i = 0; i < 4; i++, ipstr++)
    {
        ...
        if ((ipstr = strchr(ipstr, '.')) == QOSA_NULL)
        {
            break;
        }
    }
    return *(u32_t *) ipBytes;
}
                    ...
                    const struct addrinfo *hints, struct addrinfo **res)
{
    ...
    *res = QOSA_NULL;
    ...
    if ((nodename == QOSA_NULL) && (servname == QOSA_NULL))
    {
        return EAI_NONAME;
    }
    ...
    if (nodename != QOSA_NULL)
    {
        /* service location specified, try to resolve */
        if ((hints != QOSA_NULL) && (hints->ai_flags & AI_NUMERICHOST))
        {
    ...
    if (nodename != QOSA_NULL)
    {
        ...
        QOSA_ASSERT(total_size + namelen + 1 > total_size);
        total_size += namelen + 1;
    }
    /* If this fails, please report to lwip-devel! :-) */
    QOSA_ASSERT(total_size <= sizeof(struct addrinfo) + sizeof(struct sockaddr_storage) + DNS_MAX_NAME_LENGTH + 1);
...
int at_listen(int socket, int backlog)
{
    ...
    {
        if (sock->ops->at_connect(sock, QOSA_NULL, sock->sin_port, sock->type, sock->is_client) < 0)
        {
            result = -1;
            goto __exit;
        }
    ...
    /* wait the receive semaphore */
    if (qosa_sem_wait(sock->recv_notice, QOSA_WAIT_FOREVER) < 0)
    {
...
{
    struct at_socket *sock = QOSA_NULL, *news_sock = QOSA_NULL;
