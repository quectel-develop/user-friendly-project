...
#define QL_EVENT_FILE_UP_OK (1L << (8 + 8))
#define QL_EVENT_FILE_UP_FIAL (1L << (8 + 9))
#if 1
...
#endif
static struct file_device g_file_device = {0};
struct file_device *file_device_get(void)
{
    return &g_file_device;
}
...
int file_device_register(struct at_client *client)
{
    ...
    ret = qosa_event_create(&g_file_device.file_event);
    if (ret != QOSA_OK)
    {
...
int file_device_unregister(void)
{
    ...
    if (g_file_device.file_event)
    {
        ...
        qosa_event_delete(g_file_device.file_event);
    }
...
static int ql_file_event_send(struct file_device *device, u32_t event)
{
...
// This function handles the reception of events for a file device.
// It takes a pointer to a 'file_device' structure, an event code, a timeout value, and an option flag as arguments.
static int ql_file_event_recv(struct file_device *device, u32_t event, u32_t timeout, u8_t option)
{
...
// Function to upload data to the modem using QFUPL AT command
#define READ_DATA_LEN 128 // Length of data to be read in each iteration

void ql_file_uploader_func(const char *data, s32_t len)
{
    ...
    if (is_data_start)
    {
        ...
        while (1)
        {
            res = f_read(&g_file_device.file, buffer, sizeof(buffer), &bytesRead);
        ...
        if (qftpget_size != 0 && total_data_written == qftpget_size)
        {
            ...
            ql_file_event_send(device, QL_EVENT_FILE_UP_OK); // Send upload success event
        }
        else
        {
            LOG_E("Upload failed or size mismatch.");
            ql_file_event_send(device, QL_EVENT_FILE_UP_FIAL); // Send upload failure event
        }
    }
    else
    {
		LOG_W("file is already exist.");
		ql_file_event_send(device, QL_EVENT_FILE_UP_FIAL); // Send upload failure event
    }
}

// Function to upload a file to the modem using QFUPL AT command
// Parameters:
...
int ql_file_put_ex(char *localfile, char *remotefile, u32_t up_size)
{
    ...
    FRESULT res = f_open(&g_file_device.file, localfile, FA_READ);
    if (res != QOSA_OK)
    {
    ...
    // Create an AT response object with a custom self-function handler (ql_file_uploader_func)
    at_response_t resp = at_create_resp_by_selffunc(64, 0, (60000), ql_file_uploader_func);
    if (!resp)
    {
        LOG_E("Memory allocation for response object failed.\n");
        return -1;
    ...
    // Define the events to wait for
     event = QL_EVENT_FILE_UP_OK | QL_EVENT_FILE_UP_FIAL;
     ql_file_event_recv(device, event, 0, QOSA_EVENT_FLAG_OR);
    ...
    event_result = ql_file_event_recv(device, QL_EVENT_FILE_UP_OK | QL_EVENT_FILE_UP_FIAL, 60 * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR);
    if (event_result < 0)
    {
    ...
    if (event_result & QL_EVENT_FILE_UP_FIAL)
    {
...
void ql_file_write_func(const char *data, s32_t len)
{
    ...
    {
        ql_file_event_send(device, QL_EVENT_FILE_UP_OK); // Send upload success event
        LOG_V("send QL_EVENT_FILE_UP_OK");
    }
    else
    {
        ql_file_event_send(device, QL_EVENT_FILE_UP_FIAL); // Send upload success event
        LOG_V("send QL_EVENT_FILE_UP_FIAL");
    }
...
int ql_fs_write(u16_t file_handle, u16_t write_size, char *write_buffer)
{
    int result = 0;
    at_client_t client = at_client_get_first();
    file_device_register(client);
    ...
    ql_file_event_recv(device, event, 0, QOSA_EVENT_FLAG_OR);
    // Create an AT response object with a custom self-function handler (ql_file_uploader_func)
    at_response_t resp = at_create_resp_by_selffunc(64, 0, (60000), ql_file_write_func);
    if (!resp)
    {
        LOG_E("Memory allocation for response object failed.\n");
        return -1;
    ...
    // u8_t isListing = 0;
    at_exec_cmd(resp, "AT+QFWRITE=%d,%d", file_handle, write_size);
    LOG_V(">>>>> at_exec_cmd(AT+QFWRITE=file_handle, write_size);");
    // Wait for the upload result
    event_result = ql_file_event_recv(device, QL_EVENT_FILE_UP_OK | QL_EVENT_FILE_UP_FIAL, 20 * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR);
    LOG_V("+++++ ql_file_event_recv, event_result = 0x%x", event_result);
    if (event_result < 0)
    {
        result = -QOSA_ERROR_TIMEOUT;
        goto __exit;
    ...
    at_client_send(write_buffer, write_size); // Send data to the modem
    LOG_V(">>>>> at_client_send(write_buffer, write_size);");
    qosa_task_sleep_ms(500);
    event_result = ql_file_event_recv(device, QL_EVENT_FILE_UP_OK | QL_EVENT_FILE_UP_FIAL, 1000 * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR);
    LOG_V("+++++ ql_file_event_recv, event_result = 0x%x", event_result);
    if (event_result < 0)
    {
        result = -QOSA_ERROR_TIMEOUT;
...
int ql_fs_seek(u16_t file_handle, u16_t offset, u16_t pos)
{
    ...
    if (at_exec_cmd(query_resp, "AT+QFSEEK=%d,%d,%d", file_handle, offset, pos) < 0)
    {
