 ...
 */
static osa_mutex_t bg95_mqtt_lock;
...
int mqtt_device_register(struct at_client *client)
{
	...
	ret = qosa_event_create(&g_bg95_mqtt_device.mqtt_urc_event);
	if (ret != QOSA_OK)
	{
...
int mqtt_device_unregister(void)
{
	...
	if (g_bg95_mqtt_device.mqtt_urc_event)
	{
		...
		qosa_event_delete(g_bg95_mqtt_device.mqtt_urc_event);
	}
...
int bg95_mqtt_event_send(struct mqtt_device *device, u32_t event)
{
	...
	return (int)qosa_event_send(device->mqtt_urc_event, event);
}
...
static int bg95_mqtt_event_recv(struct mqtt_device *device, u32_t event, u32_t timeout, u8_t option)
{
	...
	recved = qosa_event_recv(device->mqtt_urc_event, event, option, timeout);
...
BG95_MQTT_ERROR_T QL_mqtt_open(BG95_MQTT_CONN_FD fd, BG95_MQTT_CONFIG_T *cfg_struct)
{
	...
	int event_result = 0;
	struct mqtt_device *device = QOSA_NULL;
	...
	bg95_mqtt_event_recv(device, event, 0, QOSA_EVENT_FLAG_OR);
	...
	event_result = bg95_mqtt_event_recv(device,
										BG95_EVENT_MQTT_OK | BG95_EVENT_MQTT_FIAL, 40000, QOSA_EVENT_FLAG_OR);
...
BG95_MQTT_ERROR_T QL_mqtt_dis(BG95_MQTT_CONN_FD fd)
{
	...
	/* waiting OK or failed result */
	event_result = bg95_mqtt_event_recv(device,
										BG95_EVENT_MQTT_OK | BG95_EVENT_MQTT_FIAL, 20000, QOSA_EVENT_FLAG_OR);
	...
	event_result = bg95_mqtt_event_recv(device, BG95_EVENT_MQTT_DIS_OK, 5000, QOSA_EVENT_FLAG_OR);
	if (event_result < 0)
	{
		LOG_V("bg95_mqtt_event_recv time out");
 ...
 void clean_matching_index(uint8_t idx)
{
    ...
    LOG_I("client_idx[%d] released.", idx);
}
// ----------------------------------------------------------------- END OF FILE
