...
static osa_msgq_t g_bg95_ftp_msg_id = NULL;
static osa_task_t g_bg95_ftp_thread_id = NULL;
...
int ftp_device_register(struct at_client *client)
{
    ...
    ret = qosa_event_create(&g_bg95_ftp_device.ftp_urc_event);
    if (ret != QOSA_OK)
    {
...
int ftp_device_unregister(void)
{
    ...
    if (g_bg95_ftp_device.ftp_urc_event)
    {
        ...
        qosa_event_delete(g_bg95_ftp_device.ftp_urc_event);
    }
...
static int bg95_ftp_event_send(struct ftp_device *device, u32_t event)
{
    ...
    return (int)qosa_event_send(device->ftp_urc_event, event);
}
...
static int bg95_ftp_event_recv(struct ftp_device *device, u32_t event, u32_t timeout, u8_t option)
{
    ...
    recved = qosa_event_recv(device->ftp_urc_event, event, option, timeout);
...
static void urc_ftp_open_func(struct at_client *client, const char *data, s32_t size)
{
    // Variables to hold error codes.
    int err = 0, protocol_err = 0;

    // Pointer for FTP device structure, initialized to null.
    struct ftp_device *device = QOSA_NULL;
...
static void urc_ftp_cwd_func(struct at_client *client, const char *data, s32_t size)
{
    int err = 0, protocol_err = 0;
    struct ftp_device *device = QOSA_NULL;
...
static void urc_ftp_size_func(struct at_client *client, const char *data, s32_t size)
{
    int err = 0, protocol_err = 0;
    struct ftp_device *device = QOSA_NULL;
    s32_t qftpfile_size = 0; // Local buffer to store the path
    ...
    if (sscanf(data, "+QFTPSIZE: %d", &err) > 0 && err == 0)
    {
        if (sscanf(data, "+QFTPSIZE: 0,%d", &qftpfile_size) == 1)
        {
            ...
            // device->user_data = malloc(sizeof(s32_t));
            // if (device->user_data) {
            //     *(s32_t *)device->user_data = qftpfile_size;
            //      LOG_E("Failed to allocate memory for user_data %d %d.",qftpfile_size,(s32_t *)device->user_data);
            // } else {
...
// Function to handle the URC for FTP print working directory (PWD) operation.
static void urc_ftp_pwd_func(struct at_client *client, const char *data, s32_t size)
{
    int err = 0, protocol_err = 0;
    struct ftp_device *device = QOSA_NULL;
...
static void urc_ftp_list_func(struct at_client *client, const char *data, s32_t size)
{
    int err = 0, protocol_err = 0;
    struct ftp_device *device = QOSA_NULL;
 ...
 ******************************************************************************/
s32_t ftp_send_my_msg(osa_msgq_t my_msg_id, s32_t what, s32_t arg1, s32_t arg2, s32_t arg3)
{
    s32_t ret;
    msg_node msg;
    int status = QOSA_OK;
    ...
    status = qosa_msgq_release(my_msg_id, sizeof(msg_node), (void *)&msg, 0);

    // Check if the message sending was successful or not.
    if (status != QOSA_OK)
    {
        // Log an error message if sending the message failed.
        LOG_E("Send msg to myself failed!%d", status);
...
int QL_ftp_cfg(FTP_Config *config)
{
    ...
    {
        // ql_file_del();
        at_response_t resp = at_create_resp(32, 0, (400));
        if (!resp)
        {
            LOG_E("Failed to create response object.\n");
            return -1;
...
int QL_ftp_open(FTP_Config *config)
{
    ...
    at_response_t resp = at_create_resp(32, 0, (3000));
    if (!resp)
    {
    ...
    bg95_ftp_event_recv(device, event, 0, QOSA_EVENT_FLAG_OR);
    ...
    event_result = bg95_ftp_event_recv(device,
                                       BG95_EVENT_FTP_OK | BG95_EVENT_FTP_FIAL, 20000, QOSA_EVENT_FLAG_OR);
...
int QL_ftp_path_cfg(const char *path_name)
{
    ...
    /* waiting OK or failed result */
    event_result = bg95_ftp_event_recv(device,
                                       BG95_EVENT_FTP_OK | BG95_EVENT_FTP_FIAL, 30 * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR);
    if (event_result < 0)
    {
...
// Function to get the current directory of the FTP server
s32_t QL_ftp_get_size(char *file_name)
{
    int err = 0, protocol_err = 0;
    u32_t event = 0;
    int result = 0;
...
int QL_ftp_list_get(const char *dirname, const char *local_name, FileInfo *fileList, u8_t maxFiles)
{
    ...
    at_response_t resp = at_create_resp(2048, 0, QOSA_WAIT_FOREVER);
    if (!resp)
    {
        LOG_E("No memory for response object.\n");
        return -1;
...
void bg95_ftp_client_downloader_func(const char *data, s32_t len)
{
    char read_data[READ_DATA_LEN] = {0}; // Buffer to store incoming data

    // s32_t buffered_data_len = 0;            // Length of data in the buffer
    qosa_bool_t is_data_start = QOSA_FALSE; // Flag to indicate the start of data

    s32_t total_data_written = 0; // Total data written
                                      // s32_t qftpget_size = 0;                 // Size received from "+QFTPGET:" response
    s32_t file_size = 0;          // Size received from "+QFTPGET:" response
    struct ftp_device *device = QOSA_NULL;
    ...
    if (is_data_start)
    {
        ...
        {
            s32_t read_data_len = at_self_recv(read_data, sizeof(read_data), 1000, 0);
            if (read_data_len > 0)
            {
...
void bg95_ftp_client_uploader_func(const char *data, s32_t len)
{
    ...
    s32_t qftpget_size = 0;                   // Size of the data received from QFTPPUT
    ...
    if (is_data_start)
    {
        ...
        qosa_task_sleep_ms(3000);            // Delay for 1500ms
...
int ql_ftp_client_put_ex(char *localfile, char *remotefile, QL_FTP_CLIENT_READ_CB_EX read_cb)
{
    ...
    at_response_t resp = at_create_resp_by_selffunc(64, 0, (60000), bg95_ftp_client_uploader_func);
    if (!resp)
    {
        LOG_E("No memory for response object.\n");
        return -1;
    ...
    event_result = bg95_ftp_event_recv(device, BG95_EVENT_FTP_UP_OK | BG95_EVENT_FTP_UP_FIAL, 60 * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR);
    if (event_result < 0)
    {
        result = -QOSA_ERROR_TIMEOUT;
        goto __exit;
...
int ql_ftp_client_get_ex(char *remotefile, char *localfile, QL_FTP_CLIENT_WRITE_CB_EX write_cb)
{
    ...
    // Create an AT response object with a custom self-function handler (bg95_ftp_client_downloader_func)
    at_response_t resp = at_create_resp_by_selffunc(64, 1, (60000), bg95_ftp_client_downloader_func);
    if (!resp)
    {
        LOG_E("No memory for response object.\n");
        return -1;
    ...
    if (fr != QOSA_OK)
    {
        LOG_E("Failed to open file for writing. Err[%d], localfile[%s]", fr, localfile);
        return -1;
    }
    LOG_D("Local file OPEN OK: [%s]", localfile);
    ...
    event_result = bg95_ftp_event_recv(device, BG95_EVENT_FTP_OK | BG95_EVENT_FTP_FIAL, 60 * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR);
    if (event_result < 0)
    {
        result = -QOSA_ERROR_TIMEOUT;
        goto __exit;
...
int ql_ftp_close(void)
{
    ...
    // Creating a response object for the AT command
    query_resp = at_create_resp(32, 0, (1000));
...
static void bg95_ftp_service_proc(void *argument)
{
    ...
    while (1)
    {
        ...
        status = qosa_msgq_wait(g_bg95_ftp_msg_id, (void *)&msgs, sizeof(msg_node), QOSA_WAIT_FOREVER);
        if (status != QOSA_OK)
        {
...
int bg95_ftp_service_create(void)
{
    ...
    at_client_t client = at_client_get_first();
    // 1. Creat msg queue
    status = qosa_msgq_create(&g_bg95_ftp_msg_id, sizeof(msg_node), MAX_MSG_COUNT);
    if (status != QOSA_OK)
    {
    ...
    ret = qosa_task_create(&g_bg95_ftp_thread_id, 512 * 6, QOSA_PRIORITY_NORMAL, "Ftp_S", bg95_ftp_service_proc, NULL);
    if (ret != QOSA_OK)
    {
...
int bg95_ftp_service_destroy(void)
{
    ...
    {
        ret = qosa_task_delete(g_bg95_ftp_thread_id);
        if (0 != ret)
        {
    ...
    {
        ret = qosa_msgq_delete(g_bg95_ftp_msg_id);
        if (QOSA_OK != ret)
        {
