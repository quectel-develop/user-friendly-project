...
static osa_msgq_t g_bg95_http_msg_id = NULL;
static osa_task_t g_bg95_http_thread_id = NULL;
...
#define BG95_HTTP_EVENT_AT_RECV_URC               (1L << (8 + 5)) //Add by Jerry Chen, 2025-06-24
...
static int bg95_http_event_send(ST_Http_Info_t *http_info, u32_t event)
{
    LOG_V("%s, 0x%x",__FUNCTION__, event);
    return (int) qosa_event_send(http_info->http_event, event);
}

static int bg95_http_event_recv(ST_Http_Info_t *http_info, u32_t event, u32_t timeout, u8_t option)
{
    u32_t recved;

    LOG_V("%s, event = 0x%x, timeout = %d, option = %d",__FUNCTION__, event, timeout, option);

    recved = qosa_event_recv(http_info->http_event, event, option, timeout);
    if (recved < 0)
    {
...
static void urc_qhttpget_func(struct at_client *client, const char *data, s32_t size)
{
...
static void urc_cmr_error_func(struct at_client *client, const char *data, s32_t size)
{
    int err_num_ret;
...
static void urc_httpread_func(struct at_client *client, const char *data, s32_t size)
{
    int err_num_ret;
    ...
    //Add by Jerry Chen, 2025-06-24
    g_http_info.urc_value = err_num_ret;
    bg95_http_event_send(&g_http_info, BG95_HTTP_EVENT_AT_RECV_URC);
}

static void urc_httppost_func(struct at_client *client, const char *data, s32_t size)
{
    int httprspcode_ret, err_num_ret;
...
static int configure_http_ssl(ql_SSL_Config *config)
{
    if (config->sslenble)
    {
        ...
        at_response_t query_resp = at_create_resp(64, 0, (400));
        if (!query_resp) {
...
static s32_t ql_http_param_init(ST_Http_Param_t* http_param_t)
{
    ...
    QOSA_PARAMETER_RANGE_CHECK(http_param_t->requestheader, RQue_Disable, RQue_Enable, RQue_Disable);
    at_resp_set_info(query_resp, 64, 0, (300));
    ...
    QOSA_PARAMETER_RANGE_CHECK(http_param_t->responseheader, RPon_Disable, RQon_Enable, RPon_Disable);
    at_resp_set_info(query_resp, 64, 0, (300));
    ...
    QOSA_PARAMETER_RANGE_CHECK(http_param_t->contenttype, app_urlencoded, image_jpeg, text_plain);
    at_resp_set_info(query_resp, 64, 0, (300));
    ...
    {
        at_resp_set_info(query_resp, sizeof(http_param_t->custom_header) + strlen("AT+QHTTPCFG=\"custom_header\","), 0, (300));
    ...
    QOSA_PARAMETER_RANGE_CHECK(http_param_t->timeout, 1, 65535, 60);
    QOSA_PARAMETER_RANGE_CHECK(http_param_t->rsptime, 1, 65535, 60);
    QOSA_PARAMETER_RANGE_CHECK(http_param_t->packets_space_time, 1, 65535, 60);

    return 0;
}

static int request_data_process_func(const char *data, s32_t len, const char *head_key_word, Enum_Http_Client_Event data_incoming_event, Enum_Http_Client_Event data_ok_event, Enum_Http_Client_Event data_fail_event)
{
    ST_Http_Body_t body;
    u32_t read_data_len = 0, need_read_data_len = 0, total_read_data_len = 0, need_read_count = ALIGN_UP(g_http_info.content_length, HTTP_READ_BODY_SIZE)/HTTP_READ_BODY_SIZE, i;
    int result = 0;
...
static void bg95_http_client_request_get_func(const char *data, s32_t len)
{
    int result = 0;
...
static void bg95_http_client_request_post_func(const char *data, s32_t len)
{
    int result = 0;
...
{
    int i, result = -1;
    u32_t event = 0, event_result, total_need_send_data_len = 0, read_size = 0, total_read_len = 0;
    at_response_t query_resp = NULL;
    uint8_t temp[128];
    ...
    QOSA_ASSERT(request);
    QOSA_ASSERT(request->request_url);
    QOSA_ASSERT(request->rsp_cb);
    LOG_D("request_url=%s, method=%d, request_mode = %d, username = %s, password = %s", request->request_url, request->method, request->request_mode, request->username, request->password);
    ...
    //2. Set URL
    query_resp = at_create_resp(64, 2, (g_http_info.http_param.timeout*RT_TICK_PER_SECOND));
    if (query_resp == NULL) {
        LOG_E("No memory for response object");
        return result;
    }
    ...
    at_resp_set_info(query_resp, 64, 0, (600));
    ...
    if (request->method == QL_HTTP_CLIENT_REQUEST_GET)
    {
        ...
        bg95_http_event_recv(&g_http_info, event, 0, QOSA_EVENT_FLAG_OR);
        ...
        {
            at_resp_set_info(query_resp, 64, 2, (g_http_info.http_param.rsptime*RT_TICK_PER_SECOND));
        ...
        LOG_D("wait +QHTTPGET:");
        event_result = bg95_http_event_recv(&g_http_info, event, g_http_info.http_param.rsptime * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR);
        if (event_result < 0)
        {
    ...
    else if (request->method == QL_HTTP_CLIENT_REQUEST_POST)
    {
        ...
        if (event_result < 0)
        {
            LOG_E("http wait QHTTPPOST OK|FAIL timeout.");
            goto __exit;
        }
    ...
    if ((g_http_info.err_num == 0) && (g_http_info.httprspcode == 0) && (g_http_info.content_length > 0))
    {
        at_delete_resp(query_resp);
        if (request->method == QL_HTTP_CLIENT_REQUEST_GET)
            query_resp = at_create_resp_by_selffunc(64, 1, (g_http_info.http_param.rsptime*RT_TICK_PER_SECOND), bg95_http_client_request_get_func);
        if (request->method == QL_HTTP_CLIENT_REQUEST_POST)
            query_resp = at_create_resp_by_selffunc(64, 1, (g_http_info.http_param.rsptime*RT_TICK_PER_SECOND), bg95_http_client_request_post_func);
        if (query_resp == NULL) {
            LOG_E("No memory for response object");
            result = -1;
            goto __exit;
        ...
        /* waiting OK or failed result */
        LOG_V("wait read ok");
        event_result = bg95_http_event_recv(&g_http_info, event, QOSA_WAIT_FOREVER, QOSA_EVENT_FLAG_OR);
        if (event_result < 0)
        {
        ...
        //Add by Jerry Chen, 2025-06-24
        //wait URC [+QHTTPREAD: 0]
        event_result = bg95_http_event_recv(&g_http_info, BG95_HTTP_EVENT_AT_RECV_URC, 60*RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR);
        if (event_result < 0)
        {
            LOG_E("http wait +QHTTPREAD: 0 OK|FAIL timeout.");
            result = -1;
            goto __exit;
        }
        result = g_http_info.urc_value;
    }
    else
    {
...
__exit:
    ...
    query_resp = at_create_resp(64, 0, (2000));
    if (query_resp == NULL) {
        LOG_E("No memory for response object");
        return result;
    }
...
s32_t ql_http_client_request(ST_Http_Request_t *request)
{
    ...
    u32_t event = BG95_HTTP_EVENT_REQUEST_OVER;
    s32_t ret = 0;
    ...
    {
        LOG_D("ql_http_client_request SYNC %x", qosa_event_flags_get(g_http_info.http_event));
        bcast_send_my_msg(g_bg95_http_msg_id, MSG_WHAT_BG95_HTTP_REQUEST, 0, 0, 0);
        // ret = bg95_http_event_recv(&g_http_info, event, QOSA_WAIT_FOREVER, QOSA_EVENT_FLAG_OR);
        // if (ret < 0)
        // {
        //     LOG_E("ql_http_client_request timeout event = 0x%x, %d, 0x%x", event, g_http_info.http_param.rsptime, qosa_event_flags_get(g_http_info.http_event));
        //     return -1;
        // }
        // LOG_D("ql_http_client_request SYNC over 0x%x", qosa_event_flags_get(g_http_info.http_event));
    }
    else
    {
        bcast_send_my_msg(g_bg95_http_msg_id, MSG_WHAT_BG95_HTTP_REQUEST, 0, 0, 0);
...
static void bg95_http_service_proc(void *argument)
{
    ...
    while(1)
    {
        ...
        status = qosa_msgq_wait(g_bg95_http_msg_id, (void *)&msgs, sizeof(msg_node), QOSA_WAIT_FOREVER);
		if (status != QOSA_OK)
		{
...
int bg95_http_service_create(void)
{
    ...
    //1. Creat msg queue
    status = qosa_msgq_create(&g_bg95_http_msg_id, sizeof(msg_node), MAX_MSG_COUNT);
	if (status != QOSA_OK)
	{
    ...
    ret = qosa_event_create(&g_http_info.http_event);
    if (ret != QOSA_OK)
    {
    ...
    ret = qosa_task_create(&g_bg95_http_thread_id, 512*7, QOSA_PRIORITY_NORMAL, "Http_S", bg95_http_service_proc, NULL);
	if (ret != QOSA_OK)
	{
...
int bg95_http_service_destroy(void)
{
    ...
    if (g_http_info.http_event)
        qosa_event_delete(g_http_info.http_event);
    ...
    {
        ret = qosa_task_delete(g_bg95_http_thread_id);
        if (0 != ret)
        {
    ...
    {
        ret = qosa_msgq_delete(g_bg95_http_msg_id);
        if (QOSA_OK != ret)
        {
