...
static osa_task_t g_bg95_net_thread_id =NULL;
static int QL_air_mode_set(u8_t cfun_mode);
static ip_addr_t g_module_addr = {0};
static osa_msgq_t g_bg95_net_msg_id = NULL;
static osa_timer_t   g_bg95_net_timer_id = NULL;
/******************************************************************************
* Function: net_send_msg
* Description: This function is used to send a message to the broadcast thread.
*              It uses 'qosa_msgq_release' for sending the message.
* Parameters:
...
s32_t net_send_msg(msg_node *msg)
{
    ...
    // Sending the message using qosa_msgq_release
    ret = qosa_msgq_release(g_bg95_net_msg_id, sizeof(msg_node), (void *)msg, 0);
...
static int QL_sim_state_query(void) {
    ...
    for (u8_t attempt = 0; attempt < 3; attempt++) {
        ...
        qosa_task_sleep_ms(2000); // Delay before the next attempt
    }
...
static int QL_start_datacall(void) {
    at_response_t resp;
    const char *line;
    qosa_bool_t foundQIACT1 = QOSA_FALSE;

    // Initial delay before sending the command
    qosa_task_sleep_ms(2000);

    // Creating a response object for AT command
    resp = at_create_resp(128, 0, (150*1000));  //Fix FAEDEVELOP-118
...
static int QL_check_datacall_state(void) {
    at_response_t resp;
    const char *line;
    qosa_bool_t foundQIACTQuery = QOSA_FALSE;
...
static int QL_net_state_lte_get(void) {
    ...
    // Creating a response object for AT command
    query_resp = at_create_resp(64, 0, (500));
    if (query_resp == NULL) {
        LOG_E("No memory for response object.\n");
        return -1;
    }
...
static int QL_apn_set(u8_t mode, u8_t context_id, Module_Config *operator_info) {
    at_response_t resp;
    const char *line;
    qosa_bool_t isCommandSuccess = QOSA_FALSE;
...
static int QL_cell_info_get(struct QL_CellInfo *cell_info)
{
    ...
    // Creating a response object for AT command
    resp = at_create_resp(512, 0, (2000));
    if (resp == NULL)
    {
        LOG_E("No memory for response object.\n");
        return -1;
...
static int QL_neig_info_get(QL_NeighbourCellInfo neigh_info[], int max_neighs) {
    at_response_t resp;
    resp = at_create_resp(2048, 0, (2000));
...
*              provided values.
* Parameters:
*   u32_t GSM_bandval - The band value for GSM.
*   u32_t eMTC_bandval - The band value for eMTC.
*   u32_t NB_IoT_bandval - The band value for NB-IoT.
* Return:
...
******************************************************************************/
static int QL_band_set(u32_t GSM_bandval, u32_t eMTC_bandval, u32_t NB_IoT_bandval) {
    at_response_t resp = NULL;
    ...
    if (at_exec_cmd(query_resp, "AT+QCFG=\"band\"") == 0) {
        u32_t query_GSM_bandval = 0;
        u32_t query_eMTC_bandval = 0;
        u32_t query_NB_IoT_bandval = 0;
...
static int QL_net_rat_set(QL_rat_mode mode) {
    ...
    query_resp = at_create_resp(32, 0, (2000));
...
static int QL_net_cell_lock(u8_t mode, QL_lock_cell_t *cells) {
    ...
    at_response_t query_resp = at_create_resp(64, 0, (2000));
    if (query_resp == NULL) {
        LOG_E("No memory for query response object.\n");
        at_delete_resp(resp);
        return -1;
...
int QL_module_init(AT_conn_ind_cb_t cb)
{
    ...
    ret = at_client_obj_wait_connect(client, 20000);
...
static QL_lte_mode QL_net_iot_get(void) {
    ...
    // Creating a response object for AT command
    query_resp = at_create_resp(32, 0, (300)); // Set max response time to 300ms
    if (query_resp == NULL) {
        LOG_E("No memory for response object.\n");
        return -1;
    }
...
static void QL_net_server_proc(void *argument)
{
    ...
    while(1)
    {
        ...
        status = qosa_msgq_wait(g_bg95_net_msg_id, (void *)&msgs, sizeof(msg_node), QOSA_WAIT_FOREVER);
		if (status != QOSA_OK)
		{
...
NW_START:
                ...
                {
               //     qosa_timer_start(g_bg95_net_timer_id, 20000, OSA_TRUE);
                    lte_retryCount=0;
...
int QL_bg95_net_create(Module_Config *config)
{
    ...
    //1. Creat msg queue
    status = qosa_msgq_create(&g_bg95_net_msg_id, sizeof(msg_node), MAX_MSG_COUNT);
	if (status != QOSA_OK)
	{
		LOG_E("Create qosa_msgq_create msg failed!\n");
		return -1;
	}
   // 创建定时�??
    // myTimer = osTimerNew(TimerCallback, osTimerOnce, NULL, &timer_attributes);
    //2. Creat time to keepalive
    ret = qosa_timer_create(&g_bg95_net_timer_id, checkNwCallback, NULL);
    if (ret != QOSA_OK) {
    	LOG_E("Failed to create timer.\n");
    	return -1;
    }
   // qosa_timer_start(g_bg95_net_timer_id, 1000, OSA_TRUE);
    ...
    ret = qosa_task_create(&g_bg95_net_thread_id, 512*6, QOSA_PRIORITY_NORMAL, "Net_S", QL_net_server_proc, config);
	if (ret != QOSA_OK)
	{
...
int QL_bg95_net_destroy(void)
{
    ...
    {
        ret = qosa_task_delete(g_bg95_net_thread_id);
        if (0 != ret)
        {
    ...
    if (NULL != g_bg95_net_msg_id)
     {
         ret = qosa_msgq_delete(g_bg95_net_msg_id);
         if (QOSA_OK != ret)
         {
    ...
    {
    	qosa_timer_stop(g_bg95_net_timer_id);
        ret = qosa_timer_delete(g_bg95_net_timer_id);
        if (0 != ret)
        {
