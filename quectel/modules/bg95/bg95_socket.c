...
static osa_msgq_t g_bg95_socket_msg_id = NULL;
static osa_task_t g_bg95_socket_thread_id =NULL;
...
static int bg95_socket_event_send(struct at_device *device, u32_t event)
{
    LOG_V("%s, 0x%x",__FUNCTION__, event);	
    return (int) qosa_event_send(device->socket_event, event);
}

static int bg95_socket_event_recv(struct at_device *device, u32_t event, u32_t timeout, u8_t option)
{
    u32_t recved;
 ...
 *          -5: no memory
 */
static int bg95_socket_at_connect(struct at_socket *socket, char *ip, int32_t port, enum at_socket_type type, qosa_bool_t is_client)
{
    u32_t event = 0;
...
__retry:
    ...
    bg95_socket_event_recv(device, event, 0, QOSA_EVENT_FLAG_OR);
    if (is_client)
    {
        switch (type)
        {
    ...
    if (bg95_socket_event_recv(device, SET_EVENT(device_socket, 0), 150 * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR|QOSA_EVENT_FLAG_NO_CLEAR) < 0)
    {
    ...
    event_result = bg95_socket_event_recv(device, BG95_EVENT_CONN_OK | BG95_EVENT_CONN_FAIL, 1 * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR);
    if (event_result < 0)
    {
 ...
 *          -5: no memory
 */
static int bg95_socket_at_send(struct at_socket *socket, const char *buff, size_t bfsz, char *ip, int32_t port, enum at_socket_type type, qosa_bool_t is_client)
{
    u32_t event = 0;
    int result = 0, event_result = 0;
    ...
    static osa_mutex_t lock = QOSA_NULL;//device->client->lock;
    ...
    qosa_mutex_lock(lock, QOSA_WAIT_FOREVER);//FreeRtos fifo is not supported
    ...
    while (sent_size < bfsz)
    {
        ...
        if (bg95_socket_event_recv(device, SET_EVENT(device_socket, 0), 10 * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR) < 0)
        {
            result = -QOSA_ERROR_TIMEOUT;
            goto __exit;
        }
        ...
        event_result = bg95_socket_event_recv(device,
            BG95_EVENT_SEND_OK | BG95_EVENT_SEND_FAIL, 1 * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR);
        if (event_result < 0)
        {
...
#define RESOLVE_RETRY                  3
    ...
    bg95_socket_event_recv(device, BG95_EVENT_DOMAIN_OK, 0, QOSA_EVENT_FLAG_OR);
    ...
    if (result == QOSA_OK)
    {
        for(i = 0; i < RESOLVE_RETRY; i++)
        {
            ...
            if (bg95_socket_event_recv(device, BG95_EVENT_DOMAIN_OK, 10 * RT_TICK_PER_SECOND, QOSA_EVENT_FLAG_OR) < 0)
            {
                continue;
            }
            else
...
static void urc_send_func(struct at_client *client, const char *data, s32_t size)
{
    int device_socket = 0;
    struct at_device *device = QOSA_NULL;
    struct at_device_data *bg95 = QOSA_NULL;
...
static void urc_dnsqip_func(struct at_client *client, const char *data, s32_t size)
{
    ...
    if (j == 3)
    {
        ...
        if (bg95->socket_data == QOSA_NULL)
        {
            bg95->socket_data = calloc(1, sizeof(recv_ip));
            if (bg95->socket_data == QOSA_NULL)
            {
                return;
            }
        }
        memcpy(bg95->socket_data, recv_ip, sizeof(recv_ip));
...
static void bg95_socket_service_proc(void *argument)
{
    ...
    while(1)
    {
        ...
        status = qosa_msgq_wait(g_bg95_socket_msg_id, (void *)&msgs, sizeof(msg_node), QOSA_WAIT_FOREVER);
		if (status != QOSA_OK)
		{
...
int bg95_socket_service_create(void)
{
    ...
    //1. Creat msg queue
    status = qosa_msgq_create(&g_bg95_socket_msg_id, sizeof(msg_node), MAX_MSG_COUNT);
	if (status != QOSA_OK)
	{
    ...
    ret = qosa_task_create(&g_bg95_socket_thread_id, 512*10, QOSA_PRIORITY_NORMAL, "Tcp_S", bg95_socket_service_proc, NULL);
	if (ret != QOSA_OK)
	{
...
int bg95_socket_service_destroy(void)
{
    ...
    {
        ret = qosa_task_delete(g_bg95_socket_thread_id);
        if (0 != ret)
        {
    ...
    {
        ret = qosa_msgq_delete(g_bg95_socket_msg_id);
        if (QOSA_OK != ret)
        {
