...
static int user_socket_test(socket_test_config *config)
{
    ...
    LOG_V("%s, heap size = %d", __FUNCTION__, qosa_task_get_free_heap_size());
    // Create net service
...
static int user_http_test(http_test_config *config)
{
    ...
    // Create http service
    ret = qosa_task_create(&thread_id, 256 * 40, QOSA_PRIORITY_NORMAL, "Http_T", example_http_test, (void *)config);
    if (ret != QOSA_OK)
    {
        LOG_E("thread_id thread could not start!");
        return -1;
...
#ifdef __QUECTEL_USER_FRIENDLY_PROJECT_FEATURE_SUPPORT_MQTT_S_EXAMPLE__
static osa_msgq_t g_bg95_mqtt_msg_id = NULL;
static int user_mqtt_test(mqtt_test_config *config)
{
    ...
    // 1. Creat msg queue
    status = qosa_msgq_create(&g_bg95_mqtt_msg_id, sizeof(msg_node), MAX_MSG_COUNT);
    if (status != QOSA_OK)
    {
    ...
    LOG_V("%s", __FUNCTION__);
    // Create http service
    status = qosa_task_create(&thread_id, 256 * 10, QOSA_PRIORITY_NORMAL, "Mqtt_T", example_mqtt_test, (void *)config);
    if (status != QOSA_OK)
    {
        LOG_E("thread_id thread could not start!");
        return -1;
...
void user_main(void)
{
    ...
    // init_logger(LOG_DEBUG, "log.txt");
    LOG_I("Welcome to Quectel User Friendly Project !");
    ...
    //2. Create msg queue
    status = qosa_msgq_create(&g_main_msg_id, sizeof(msg_node), MAX_MSG_COUNT);
    if (status != QOSA_OK)
    {
    ...
    //4. Start debug service
    ret = debug_service_create();

    //5. AT init
    at_client_init(128, 128);

    //6. Initialize broadcast service
    ...
    //11. Register which broadcasts need to be processed
    ...
    while (1)
    {
        ...
        status = qosa_msgq_wait(g_main_msg_id, (void *)&msgs, sizeof(msg_node), QOSA_WAIT_FOREVER);
        if (status != QOSA_OK)
        {
        ...
        else
        {
            ...
            case MSG_WHAT_BG95_NET_DATACALL_SUCCESS:
                ...
                serial_input_parse_thread_wake_up();  //Jerry Chen add, 2025-06-18
                break;
...
int user_main_test(s32_t argc, char *argv[])
{
    ...
    if ((argc == 0) || (strcmp((const char *)argv[0], "help") == 0 && argv[1] == NULL))
    {
        LOG_I("---------------------------------------------------------------------------------------------------");
        LOG_I("| main:                                                                                            |");
        LOG_I("---------------------------------------------------------------------------------------------------|");
    ...
    else if (strcmp((const char *)argv[0], "mqtt") == 0)
    {
        ...
        if (mqtt_config.mqtt_test_type == 1)
        {
            ...
            LOG_I("server_type %d", mqtt_config.conf_local_hq.server_type);
    ...
    else if (strcmp((const char *)argv[0], "getversion") == 0)
    {
        // LOG_I("The current version is: %s", __QUECTEL_USER_FRIENDLY_PROJECT_VERSION);
        LOG_I("Current version: %s", QUECTEL_PROJECT_VERSION);
    }
    else
    {
