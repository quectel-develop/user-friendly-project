...
int user_fs_test(void *argument)
{
    ...
    else if (config->fs_type == 4) // WRITE
    {
        QL_fs_write(config->file_handle, config->wirte_read_size, config->wirte_buffer);
    }
    else if (config->fs_type == 5) // CLOSE
    {
        QL_fs_close(config->file_handle);
    }
    else if (config->fs_type == 6) // READ
    {

        u16_t read_len = QL_fs_read(config->file_handle, config->wirte_read_size, data);
        if (read_len > 0)
        {
